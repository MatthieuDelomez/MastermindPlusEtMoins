 ...
 * jour les observateurs. Cette classe implémente l'interface Observable.
 * 
...
public class DonneeMaster implements Observable {
	...
	/**
	 * Appel à une liste chainée contenant la liste de l'ensemble des combianaisons
	 * possibles.
	 */
	...
	private void analyseProposiManChallenger() {
		...
		// Pour faciliter le traitement, on va dire
		...
		// correspondance avec les couleurs dans l'ordre suivant :
	 ...
	 * 
	 * Suivant la reponse que fera le joueur, la liste sera réduite au fur et à
	 * mesure.
	 * 
	...
	private void propoCpuDefenseur() {
		...
		if (reponseManDefenseur.equals("")) {
			...
			LOGGER.debug("Jeu Mastermind en mode Défenseur - Proposition de l'ordinateur en mode défenseur :"
					+ propoCpuDefenseur);
			...
			LOGGER.debug(
					"Jeu Mastermind en mode Défenseur - Modele de données réponse du joueur :" + reponseManDefenseur);
			...
			LOGGER.debug("Jeu Mastermind en mode Défenseur - Taille liste chaînée réactualisée :"
					+ listePossibilitees.size());

			LOGGER.debug(
					"Jeu Mastermind en mode Défenseur - Premier élément réactualisé :" + listePossibilitees.getFirst());
	...
	/**
	 * Méthode relative au mode Duel qui permet d'analyser la proposition du joueur
	 * en la comparant à la combinaison secrète de l'ordinateur.
	 */
	...
	/**
	 * 
	 * Méthode relative au mode Duel qui permet de déterminer la proposition de
	 * l'ordinateur
	 * 
	 * en fonction de la réponse du joueur. La première proposition de l'ordinateur
	 * correspondra
	 * 
	 * au premier élément de la liste chainée initiale qui comprend toutes les
	 * possibilités
	 * 
	 * vu que le joueur n'a pas encore répondu. Par la suite, suivant la réponse du
	 * joueur, la liste
	 * 
	 * chainée sera réduite au fur et à mesure jusqu'à ne comprendre qu'un seul
	 * élément.
	 * 
	...
	public void propoCpuDuel() {
		...
		if (reponseManDuel.equals("")) {
			...
			LOGGER.debug(
					"Jeu Mastermind en mode Duel - Taille liste chaînée réactualisé :" + listePossibilitees.size());
	...
	/**
	 * Mutateur commun à tous les modes de jeu qui permet de modifier le nombre de
	 * cases.
	 * 
	...
	/**
	 * Méthode permettant d'initialiser la liste chainée avec l'ensemble des
	...
	public void initListePossibilitees() {
		...
		/*
		 * On crée un objet LinkedList avec l'ensemble des possibilités. Dans le cas où
		 * on a 4 cases et 6 couleurs utilisables,
		 * 
		 * l'objet LinkedList contiendra 1296 éléments. On s'assure bien que cette liste
		 * est initialisée à chaque début de partie
		 */
					...
					for (int k = 0; k < this.nbCouleur; k++) {
						...
						for (int l = 0; l < this.nbCouleur; l++) {
							...
							for (int m = 0; m < this.nbCouleur; m++) {
								...
								listePossibilitees.add(String.valueOf(i) + String.valueOf(j) + String.valueOf(k)
