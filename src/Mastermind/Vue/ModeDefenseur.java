...
public class ModeDefenseur extends JPanel implements Observer {
	...
	private ImageIcon imgEmplacementVide = new ImageIcon("images/EmplacementVide.png"),
			...
			imgEmplacementVideSoluceCombiSecreteMan = new ImageIcon(
	...
	public ModeDefenseur(int nbCase, int nbEssai, int nbCouleur, boolean modeDeveloppeurActive,
		...
		default:
			LOGGER.error(
					"Jeu en mode Defenseur - Erreur lors de la mise en place de l'IHM pour les boutons liés aux couleurs");
	...
	/*
	 * Pattern Observer - Méthode pour MAJ la grille de jeu selon la proposition du
	 * Cpu.
	 * 
	...
	public void updateMaster(String propoCpu) {
		...
		/*
		 * Nous devons convertir la proposition du Cpu en JLabel avc les img
		 * correspondantes sur le grille.
		 */
		containerGrilleJeu.removeAll();
		for (int i = 0; i < this.nbCase; i++) {
			...
			default:
				LOGGER.error(
						"Jeu en mode Défenseur - Erreur de correspondance entre la proposition du cpu et les couleurs");
			}
		}
	...
	/*
	 * Patter Observer - Méthode pour relancer le même jeu : Réinitialisation de
	 * l'IHM et de la grille de jeu et la combinaison secrète en bas de page + les
	 * variables.
	 */
	...
	private void affichageReponseMan(ImageIcon couleurChoisieReponse, int codeCouleurReponse) {
		if (remplissageSoluce < this.nbCase) {
			...
			/**
			 * On réordonne le tablea d'entiers dans l'ordre numérique puis on affectue la
			 * correspondance avec les couleurs dans l'ordre suivant : - Pions rouge (si
			 * présents) - Pions blanc (si présent) - Emplacement vide
			 */
